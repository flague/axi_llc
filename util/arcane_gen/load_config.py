# From https://github.com/esl-epfl/x-heep/blob/main/util/x_heep_gen/load_config.py
...
from .system import BusType, Override, Arcane
...
def load_ram_configuration(system: Arcane, mem: hjson.OrderedDict):
    """
    Reads the whole ram configuration.

    :param Arcane system: the system object where the ram should be added.
    ...
    :raise RuntimeError: when an invalid configuration is processed.
    """
    if not isinstance(system, Arcane):
        raise TypeError("system should be an instance of Arcane object")
    ...
    for key, value in mem.items():
        ...
        # Memory banks are always interleaved
...
def load_linker_config(system: Arcane, config: list):
    """
    Reads the whole linker section configuration.

    :param Arcane system: the system object where the sections should be added.
...
def load_peripherals_config(system: Arcane, config_path: str):
    """
    Reads the whole peripherals configuration.

    :param Arcane system: the system object where the peripherals should be added.
...
def load_cfg_hjson(src: str, override: Optional[Override] = None) -> Arcane:
    """
    ...
    system = Arcane(BusType(bus_config), ram_start, override=override)
...
def load_cfg_hjson_file(f: PurePath, override: Optional[Override] = None) -> Arcane:
    """
...
def load_cfg_script_file(f: PurePath) -> Arcane:
    """
    Executes the python file passed as argument to cinfigure the system.

    This file should have a function config that takes no parameters and returns an instance (or subclass) of the Arcane type.
...
def load_cfg_file(f: PurePath, override: Optional[Override] = None) -> Arcane:
    """
